...
public class StepsTrackerDBHelper extends SQLiteOpenHelper {
    ...
    private static final String CREATE_TABLE_STEPS_SUMMARY = "CREATE TABLE " +
            ...
            STEP_DATE + " TEXT," + SESSION_ID + " TEXT," + STEP_TIME + " INTEGER," +
            STEP_TYPE + " TEXT" + ")";
    ...
    public int[] getStepsByDate(String date) {
        ...
        String selectQuery = "SELECT " + STEP_TYPE + " FROM " + TABLE_STEPS_SUMMARY +
                " WHERE " + STEP_DATE + " = '" + date + "'";
        try {
    ...
    public long getTotalStepsDuration() {
        ...
        try {
            ...
            for (int i = 0; i < sizeSessionNameList; i++) {
                ...
                String selectTimeQuery = "SELECT " + STEP_TIME + " FROM " + TABLE_STEPS_SUMMARY +
                        " WHERE " + SESSION_ID + " = '" + sessionNameList.get(i) + "'";
                ...
                for (int j = sizeStepTimeSessionList - 1; j == 1; j--) {
                    totalDuration = totalDuration + stepTimeSessionList.get(j) -
                            stepTimeSessionList.get(j - 1);
                }
            }
