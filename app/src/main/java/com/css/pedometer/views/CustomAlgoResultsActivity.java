...
public class CustomAlgoResultsActivity extends Activity {
    ...
    public void calculateDataMatrix() {
        ...
        String todayDate = String.valueOf(calendar.get(Calendar.MONTH) + 1) + "/" +
        ...
        mTotalDurationTextView.setText(String.format("%.0f", hours) + " hrs " +
                String.format("%.0f", minutes) + " mins " + String.format("%.0f", seconds) + " secs");

        //Calculating average speed
        if (totalDistance > 0) {
            mAverageSpeedTextView.setText(String.format("%.2f", totalDistance / totalDuration) +
                    " meter per seconds");
        } else {
        ...
        if (totalStepTaken > 0) {
            mAveragFrequencyTextView.setText(String.format("%.0f", totalStepTaken / minutes) +
                    " steps per minute");
        } else {
